import { generateWorkspaceInviteLink } from "@/actions/create-workspace-invite-link";
import { deleteWorkspaceInviteLink } from "@/actions/delete-workspace-invite-link";
...
export const WorkspaceShareLink = () => {
  ...
  const { execute: executeDeleteInviteLink } = useAction(
  ...
  const handleDeleteLink = () => {
    if (inviteLink) {
      ...
      if (token) {
        executeDeleteInviteLink({ workspaceId, token });
      }
    }
  };

  const { execute: executeGenerateInviteLink } = useAction(
  ...
  const handleGenerateInviteLink = () => {
    executeGenerateInviteLink({ workspaceId });
  };
  if (isLoading) {
    return <WorkspaceShareLinkSkeleton />;
  }
...
const WorkspaceShareLinkSkeleton = () => {
  return (
    <div className="p-6 bg-zinc-800 rounded-lg shadow-lg transition-all duration-300">
      <div className="flex items-center mb-4">
